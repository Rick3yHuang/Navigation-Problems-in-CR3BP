-- Problem: M+3S all ranges (Lyapunov quartic)
...
cA = makeCubics(#lyapQMons(xA, yA), CA)
cB = makeCubics(#lyapQMons(xB, yB), CB)
cC = makeCubics(#lyapQMons(xC, yC), CC)
gA = buildGcoeffs(lyapQMons(xA, yA), cA)
gB = buildGcoeffs(lyapQMons(xB, yB), cB)
gC = buildGcoeffs(lyapQMons(xC, yC), cC)
eqs = {
    gA, gB, gC,
