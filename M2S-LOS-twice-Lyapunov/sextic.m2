-- Problem: M2S LOS two time instances (Lyapunov sextic)
...
-- Expected degree: 2592  (WARNING: very slow)
...
cA = makeCubics(#lyapSMons(xA1, yA1), CA)
cB = makeCubics(#lyapSMons(xB1, yB1), CB)
gA = (xx, yy) -> buildGcoeffs(lyapSMons(xx, yy), cA)
gB = (xx, yy) -> buildGcoeffs(lyapSMons(xx, yy), cB)
eqs = {
